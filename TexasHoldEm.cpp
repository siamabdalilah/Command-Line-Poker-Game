/**
 *	TexasHoldEm.h
 *	Function: The functions declared in TexasHoldEm.h
...
void TexasHoldEm::second_turn() {
	unsigned int siz = players.size();
	int offset = (dealer + 1) % siz;
	...
	for (int j = 0; j < 3; ++j) {
		...
		Card c = deck.getDeck().back();
		c.faceDown = false;
		for (unsigned int i = 0; i < siz; i++) {
			players[i]->hand.add_card(c);
		}
